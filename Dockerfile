...
RUN yum -y install git gcc gcc-c++ bison flex zlib-devel openssl-devel mysql mysql-server mysql-devel subversion pcre-devel ncurses-devel ncurses openldap openldap-devel
...
RUN git clone https://github.com/OpenSIPS/opensips.git -b 2.2 ~/opensips_2_2 && \
    ...
    sed -i 's/json ldap lua/json lua/g' ~/opensips_2_2/Makefile.conf.template && \
    cd ~/opensips_2_2 && \
